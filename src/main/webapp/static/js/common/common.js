...
//	});
	...
	$('.datagrid-view').live('keyup',function(e){
		...
		if($(e.target).hasClass('combo-text')){
			//不是时间输入框
			if(!$(e.target).parent().hasClass('datebox')){
				return;
			}
		}
		...
		if(e.keyCode == 38){
			...
			if(!table.datagrid('options').enterDown){
				return;
			}else{
				// 确定键
				if(e.keyCode == 13){
		...
		if($(e.target).hasClass('easyui-timespinner')){
			// numberbox提前触发blur事件
			$(e.target).blur();
		}
		...
		if(table.datagrid('isEndRowEditing')){
			...
			// 一般画面用
			if(table.datagrid('options').suppReturnFlag){
				suppReturnDetailEditRow(newEdit);
				
			}else if(table.datagrid('options').stocktakeFlag){
				// 盘点画面用
				stocktakeDetailEditStocktakeDetailRowSet(table,newEdit,editField);
				
			}else if(table.datagrid('options').shopReturnFlag){
				shopReturnDetailEditRow(newEdit);
				/!*table.datagrid('exEditCellForProfitLoss',{index:newEdit,field:'qty',editField:['qty','returnReason','returnReasonText','taxAmount','amountOrignal','sellAmountTaxoff']});
				var rowData = table.datagrid('getRows')[newEdit];
				var qtyTarget = $(table.datagrid('getEditor',  {index:newEdit,field:'qty'}).target);
				qtyTarget.numberbox('options').max = rowData.maxQty;*!/
			}else{
				if(editField){
					// 定位到当前编辑列
...
*/
	/* 在原始事件植入自己的代码，这种的做法不太好，
	 * 请参考easyui-defaults-settings的combogrid的down事件，
	 * 今后不再在这里写了   
	 */
	// enter 转 tab
	...
	if ($.fn.combobox) {
		(function(){
			...
			gridEnter = gridEnter?gridEnter:function(e){};
			...
			$('.combo-text').live('blur', function(e){
				...
				if ($(cmb).is('.easyui-combobox') ) {
					if(!$(cmb).is('.freeinput')){
						//oldf.call(cmb, e);
					}
					options = $(cmb).combobox('options');
				}
				if (($(cmb).is('.easyui-combogrid') || $(cmb).is('.combogrid-f'))) {
					if(!$(cmb).is('.freeinput')){
	...
	// 出库复验画面文本框，只能输入数字 change 事件
	$(".exp-check-list-text-only-number").live('change',function() {
		// 把文本框内0到9以外的清除掉
	...
	// 重写deleteRow， 当前删除的行，如果不是新增的，曾经改变过，则在updatedRows去掉该行
	if ($.fn.datagrid){
		if(!$.fn.datagrid.defaults.PreDeleteRow){//将原生的deleteRow存起来
			$.fn.datagrid.defaults.PreDeleteRow = $.fn.datagrid.methods.deleteRow;
		}
		//将deleteRow方法覆盖掉，首先调用原先的deleteRow再扩充
		$.fn.datagrid.methods.deleteRow = function(jq,index){
			// 取得已经修改的列表
			var updatedRows=$.data(jq[0],"datagrid").updatedRows;
			// 取得当前要删除的行的数据
			var row=$.data(jq[0],"datagrid").options.finder.getRow(jq[0],index);
			var updateIndex = -1;
			// 查找修改列表中是否有当前行
			...
			// 执行原来的删除操作
			$.fn.datagrid.defaults.PreDeleteRow.call(this,jq,index);
		};
	}
